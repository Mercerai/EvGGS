...
class PatchSplitting(BaseModule):
    """
    """
    ...
    def __init__(self,
        ...
        Upsample_dim = stride**2 * out_channels
        self.increase = nn.Linear(in_channels, Upsample_dim, bias=bias)

        self.Upsampler = nn.Fold(
            output_size=output_size, kernel_size=stride, dilation=1, padding=0, stride=stride)
    ...
    def forward(self, x):
        ...
        x = x.transpose(1, 2)  # B, 2*C, H*W
        B, C, L = x.shape
        
        if (oh % self.stride != 0) or (ow % self.stride != 0):
            H = int(math.ceil(oh / self.stride))
            W = int(math.ceil(ow / self.stride))
            x = x.view(B, C, H, W)
            x = x[:, :, :(oh // self.stride), :(ow // self.stride)]
            x = x.reshape(B, C, -1)
        
        x = self.Upsampler(x)  # B, C/2, 2H, 2W
        x = x.permute([0, 2, 3, 1]).view(B, \
            oh*ow, self.out_channels) #B, 2H*2W, C/2
        
        x = self.norm(x) if self.norm else x

        # up_hw_shape = self.output_size
        return x
...
class WindowMSA(BaseModule):
    ...
    def double_step_seq(self, step1, len1, step2, len2):
        seq1 = torch.arange(0, step1 * len1, step1)
        seq2 = torch.arange(0, step2 * len2, step2)
        return (seq1[:, None] + seq2[None, :]).reshape(1, -1)

class CrossWindowMSA(BaseModule):
    """Window based multi-head self-attention (W-MSA) module with relative
    position bias.
    ...
    def forward(self, x_q, x_a, mask=None):
        """
        Args:
        ...
        q = self.q(x_q).reshape(B, N, self.num_heads, 
                                C // self.num_heads).permute(0, 2, 1, 3)
        k, v = self.kv(x_a).reshape(B, Na, 2, self.num_heads,
                                      C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class CrossShiftWindowMSA(BaseModule):
    """Shift Window Multihead Self-Attention Module.
    ...
    def forward(self, query, key, hw_shape):
        B, L, C = query.shape
        H, W = hw_shape
        assert L == H * W, 'input feature has wrong size'
        query = query.view(B, H, W, C)
        ...
        # nW*B, window_size, window_size, C
        query_windows = self.window_partition(shifted_query)
        key_windows   = self.window_partition(shifted_key)
        # nW*B, window_size*window_size, C
        query_windows = query_windows.view(-1, self.window_size**2, C)
        key_windows   = key_windows.view(-1, self.window_size**2, C)

        # W-MSA/SW-MSA (nW*B, window_size*window_size, C)
        attn_windows = self.w_msa(query_windows, key_windows, mask=attn_mask)
...
class CrossSwinBlock(BaseModule):
    """"
    Args:
        embed_dims (int): The feature dimension.
        num_heads (int): Parallel attention heads.
    ...
    def __init__(self,
        ...
        super(CrossSwinBlock, self).__init__()
    ...
    def forward(self, query, key, hw_shape):
        ...
        key   = self.norm2(key)
        query = self.attn(query, key, hw_shape)
